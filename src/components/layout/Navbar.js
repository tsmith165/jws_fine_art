...
const Navbar = ({most_recent_page_id, app_state, app_set_state, isLoaded, isSignedIn, user }) => {
    ...
    console.log(`Rendering Navbar with app_state URL Path: ${app_state.pathname} | Theme: ${app_state.theme} | Filter Menu Open: ${app_state.filter_menu_open}`)
    ...
    for (var i = 0; i < THEME_FILTERS.length; i++) {
        ...
        filter_menu_array.push((
            <div className={(filter == app_state.theme) ? `${styles.gallery_filter_icon_container_selected} ${styles.gallery_filter_icon_container}` : styles.gallery_filter_icon_container} 
                ...
                onClick={(e) => { e.preventDefault(); app_set_state({theme: filter, url_path: app_state.url_path, filter_menu_open: true, pathname: app_state.pathname}) }}
            >
                {icon}
    ...
    return (
        <nav className={styles.navbar}>
            <div className={styles.navbar_container}>
                ...
                {(app_state.pathname == '/' && app_state.filter_menu_open == true) ? (
