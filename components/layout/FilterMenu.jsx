        ...
        for (var i = 0; i < THEME_FILTERS.length; i++) {
            ...
            filter_menu_array.push(
                <div
                    ...
                    id={filter}
                    className={
                        filter == appState.theme
                            ? `${filter_menu_styles.filter_icon_container_selected} ${filter_menu_styles.filter_icon_container}`
                            : filter_menu_styles.filter_icon_container
                    }
                    data-tooltip-content={`${filter}`}
                    onClick={(e) => {
                        e.preventDefault();
                        setAppState({ ...appState, theme: filter });
                    }}
                >
                    {icon}
                ...
                <div
                    className={
                        appState.filter_menu_open == false
                            ? filter_menu_styles.filter_menu_toggle
                            : `${filter_menu_styles.filter_menu_toggle} ${filter_menu_styles.filter_menu_toggle_open}`
                    }
                >
                    {appState.filter_menu_open == true ? null : (
                        <div
                            className={filter_menu_styles.filter_menu_tooltip}
                            onClick={(e) => {
                                e.preventDefault();
                                setAppState({ ...appState, filter_menu_open: !appState.filter_menu_open });
                            }}
                        >
                            Filters
                        </div>
                    ...
                    <Tune
                        className={filter_menu_styles.filter_menu_toggle_icon}
                        onClick={(e) => {
                            e.preventDefault();
                            setAppState({ ...appState, filter_menu_open: !appState.filter_menu_open });
                        }}
