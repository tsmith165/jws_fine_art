...
const Piece = ({ dimensions, id, className, o_id, image_path, title, sold, available }) => {
    ...
    return (
        <div id={id} className={`absolute p-1 rounded-md bg-tertiary ${className}`} style={{width: img_width + 8, height: img_height + 8, top: y, left: x}}>
                ...
                <div className={`p-0`} style={{width: img_width}}>
                    ...
                    (sold === true) ? (
                        <div className={`absolute !bottom-2.5 right-2.5`}>
                            <Image className={`!top-auto !left-auto`} src="/redDot.png" alt="Piece Sold" width={30} height={30} priority={true}/> 
                        </div>
                    ) : ( 
                        (USING_YELLOW_DOT && available === false) ? ( 
                            <div className={`absolute !bottom-2.5 right-2.5`}>
                                <Image className={`!top-auto !left-auto`} src="/yellowDot.png" alt="Piece Sold" layout='fixed' width={30} height={30}/> 
                            </div>
                        ) : (
                            null
                        ))
